...
# Y = C + I + G + Nx
# C = C0+B1 (Y-T)
# I = I0+B1_Y+B_Interest
# G & N exogenous (able to adjust)
# M / P = M + D1_Y + D2_Interest Rate

ISLMdata = data.frame(
  
  # consumpton  
  Consum = c(13104.43333, 13212.5, 13345.06667,13586.26667, 
             13728.33333, 13939.83333,14114.53333, 14211.9, 
             14266.26667, 14511.16667, 14678.2, 14789.26667),
  ...
  M1 = c(3405.461538, 3498.1, 3570.161538, 3613.338462,
         3639.484615, 3658.723077, 3688.038462, 3719.442857, 
         3743.891667, 3796.215385, 3867.964286, 3950.738462),
  
  # M2 money supply  M1 + easily convertible               
  M2 = c(13332.09231, 13515.83846, 13659.85385, 13794.4, 
         13892.84615, 14045.68462, 14186.1, 14275.91429, 
         14464.79167, 14645.46154, 14932.30714, 15243.2),
  ...
  # velocity of money; how many times $ changes hands per year                
  Vel_M1 = c(5.623, 5.529, 5.493, 5.513, 
             5.534, 5.605, 5.625, 5.616, 
             5.636, 5.613, 5.564, 5.5),
  ...
  # consumer price index ($100 from original amount to current)                
  CPI = c(243.822, 244.0543333, 245.359, 247.25,
          249.2346667, 250.591, 251.8826667, 252.6973333, 
          253.2753333, 255.1706667, 256.3246667, 257.8323333),
  # interest rate, %                
  fedRate = c(0.7,0.95, 1.153, 1.203, 
              1.446, 1.737,1.923, 2.22, 
              2.403, 2.397, 2.19, 1.643),
  # libor %
  libor=c(0.829557031, 1.06296541, 1.231578281,1.330114127, 
          1.653367302, 1.971670645, 2.107461406, 2.34698, 
          2.49826381, 2.442525738, 2.178625385, 1.791037656 ),
...
pIndex = CPI/CPI[1] # normalize CPI; $1 begiining of 2017 to each Qtr after
...
#    If these are different from the present levels,  
